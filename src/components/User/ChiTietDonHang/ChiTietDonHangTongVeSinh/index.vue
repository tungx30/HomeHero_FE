<template>
    <div class="d-flex justify-content-center align-items-center min-vh-100">
        <div class="card mb-4 shadow-sm" style="width: 100%; max-width: 600px;">
            <div class="card-body">
                <h5 class="card-title text-center mb-4">Đơn dịch vụ Tổng Vệ Sinh</h5>
                <div class="table-responsive">
                    <table class="table table-striped table-bordered">
                        <tbody>
                            ...
                            <tr>
                                <th>Loại Nhà</th>
                                <td>{{ chiTietDonHang.loai_nha }}</td>
                            </tr>
                            <tr>
                                <th>Diện tích + Số nhân viên</th>
                                <td>
                                    <span v-if="chiTietDonHang.dien_tich_tong_san === 1">
                                        Dưới 60m² tổng sàn + 2 nhân viên
                                    </span>
                                    <span v-else-if="chiTietDonHang.dien_tich_tong_san === 1.5">
                                        60-90m² + 3 nhân viên
                                    </span>
                                    <span v-else-if="chiTietDonHang.dien_tich_tong_san === 2">
                                        Trên 90m²-120m² + 4 nhân viên
                                    </span>
                                    <span v-else-if="chiTietDonHang.dien_tich_tong_san === 2.5">
                                        120-150m² + 5 nhân viên
                                    </span>
                                    <span v-else-if="chiTietDonHang.dien_tich_tong_san === 3">
                                        150-180m² + 6 nhân viên
                                    </span>
                                    <span v-else-if="chiTietDonHang.dien_tich_tong_san === 3.5">
                                        180-210m² + 7 nhân viên
                                    </span>
                                    <span v-else>
                                        Không xác định
                                    </span>
            ...
            axios
                ...
                .then((res) => {
                    if (res.data.status) {
                        ...
                        // Nếu có danh sách nhân viên, gán vào chiTietDonHang.nhanVien
                        if (this.chiTietDonHang.nhanViens && this.chiTietDonHang.nhanViens.length > 0) {
                            // Giả sử bạn chỉ hiển thị thông tin của nhân viên đầu tiên
