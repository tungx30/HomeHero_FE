        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body text-center">
                    <h4 class="text-center mb-4">Giúp Việc Định Kỳ </h4>
                    ...
                    <!-- <div class="section mb-3">
                        ...
                        <div class="employee-selection">
                            <button class="employee-btn ">1 x Nhân viên</button>
                            <button class="employee-btn">2 x Nhân viên</button>
                            <button class="employee-btn">3 x Nhân viên</button>
                        </div>
                    </div> -->
                    <div class="section mb-3">
                        <p class="section-title">
                    ...
                    <div class="section mb-3">
                        <p class="section-title">
                            <i class="fas fa-clock text-success"></i> Lịch Làm Hằng Tuần
                        </p>
                        <div class="days-of-week">
                            <button @click="toggleDay(1)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(1) }">
                                <p>T2</p>
                            </button>
                            <button @click="toggleDay(2)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(2) }">
                                <p>T3</p>
                            </button>
                            <button @click="toggleDay(3)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(3) }">
                                <p>T4</p>
                            </button>
                            <button @click="toggleDay(4)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(4) }">
                                <p>T5</p>
                            </button>
                            <button @click="toggleDay(5)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(5) }">
                                <p>T6</p>
                            </button>
                            <button @click="toggleDay(6)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(6) }">
                                <p>T7</p>
                            </button>
                            <button @click="toggleDay(0)"
                                :class="{ 'day-btn': true, 'active': donHang.so_ngay_phuc_vu_hang_tuan.includes(0) }">
                                <p>Cn</p>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="section mb-3">
                        <p class="section-title">
                            <i class="fas fa-archive text-success"></i> Chọn Gói
                        </p>
                        <div class="employee-selection">
                            <button @click="donHang.so_thang_phuc_vu = 3"
                                :class="{ 'employee-btn': true, 'active': donHang.so_thang_phuc_vu === 3 }">
                                <p>3 Tháng</p>
                            </button>

                            <button @click="donHang.so_thang_phuc_vu = 6"
                                :class="{ 'employee-btn': true, 'active': donHang.so_thang_phuc_vu === 6 }">
                                <p>6 Tháng</p>
                            </button>

                            <button @click="donHang.so_thang_phuc_vu = 12"
                                :class="{ 'employee-btn': true, 'active': donHang.so_thang_phuc_vu === 12 }">
                                <p>12 Tháng</p>
                            </button>

                            <button @click="donHang.so_thang_phuc_vu = 18"
                                :class="{ 'employee-btn': true, 'active': donHang.so_thang_phuc_vu === 18 }">
                                <p>18 Tháng</p>
                            </button>
                        </div>
                    </div>
                    <div class="section mb-3">
                        ...
                        <div class="employee-selection">
                            ...
                            <button @click="donHang.phuong_thuc_thanh_toan = 2"
                    ...
                    <div class="section mb3">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button v-on:click="apDungCode()" class="btn btn-outline-success-1 small-button custom-btn"  type="button"
                                id="button-addon2">Áp
                                Dụng</button>
                        </div>
                    </div>
...
export default {
    ...
    watch: {
        'donHang.so_ngay_phuc_vu_hang_tuan': function () {
            this.tinhTongTien();
        },
        'donHang.so_thang_phuc_vu': function () {
            this.tinhTongTien();
        }
    },
    methods: {
        toggleDay(day) {
            const index = this.donHang.so_ngay_phuc_vu_hang_tuan.indexOf(day);
            if (index > -1) {
                // Nếu ngày đã có trong mảng, loại bỏ nó (bỏ chọn)
                this.donHang.so_ngay_phuc_vu_hang_tuan.splice(index, 1);
            } else {
                // Nếu ngày chưa có trong mảng, thêm nó vào (chọn)
                this.donHang.so_ngay_phuc_vu_hang_tuan.push(day);
            }
        },
        generateTimeOptions() {
            const options = [];
        ...
        tinhTongTien() {
            // Tính số buổi hàng tháng (số ngày phục vụ hàng tuần * 4 tuần)
            let soBuoiHangThang = Array.isArray(this.donHang.so_ngay_phuc_vu_hang_tuan) ? this.donHang.so_ngay_phuc_vu_hang_tuan.length * 4 : 0;            // Tính tổng số buổi (số buổi hàng tháng * số tháng phục vụ)
            let tongSoBuoi = soBuoiHangThang * (this.donHang.so_thang_phuc_vu ?? 1);
            // Tính tổng tiền (số tiền mặc định * tổng số buổi)
            this.tong_tien = this.soTienMacDinh * tongSoBuoi;
...
<style scoped>
.custom-btn {
        padding: 2px 6px; /* Giảm khoảng cách bên trong */
